...
// 있도록 중간 단계를 연결해 준다.
//
// CGxAxis_YMC : Paix NMC2를 통해 구현되는 실제 Axis class
//
// 2014-12-21, jhLee
...
#include "Motion\GxMotionCmd_YMC.h"		// NMC2 API Wrapper class


class _GAUS_LIB CGxAxis_YMC : public CGxAxis
{
public:
	CGxAxis_YMC(void);
	~CGxAxis_YMC(void);
	...
	CGxMotionCmd_YMC*	m_pMyCmd;					// 실제 동작세 사용할 Cmd : 공통에서 캐스팅 치환 시킨다.
	...
	// Torque를 읽어온다. MP2100만의 기능
	virtual double GetTorque();				// Torque값 조회
	...
	BOOL	SetJogMoveSpeed(double dbAcc, double dbDec, double dbDrive);		// Jog 이동 속도 설정 (사다리꼴)
